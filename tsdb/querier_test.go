...
func expandChunkIterator(it storage.ChunkIterator) (chks []chunks.Meta, err error) {
	for it.Next() {
...
func TestBlockQuerier(t *testing.T) {
	...
	for _, tcase := range []struct {
		...
		Outer:
			for _, c := range tcase.queries {
				ir, cr, _, _ := createIdxChkReaders(t, data)
				querier := &blockQuerier{
					index:      ir,
					chunks:     cr,
					tombstones: tcase.tombstones,

					mint: c.mint,
					maxt: c.maxt,
				}

				res, _, err := querier.Select(nil, c.ms...)
				testutil.Ok(t, err)
...
func TestSeriesIterators(t *testing.T) {
	...
	t.Run("Chunk", func(t *testing.T) {
		...
		for i, tc := range append(cases, limitedMinMaxtCases...) {
			t.Run(fmt.Sprintf("%v", i), func(t *testing.T) {
				tc.test(t, newChunkSeriesIterator([]chunks.Meta{
	...
	t.Run("Vertical", func(t *testing.T) {
		...
		for i, tc := range append(cases, overlappingCases...) {
			t.Run(fmt.Sprintf("%v", i), func(t *testing.T) {
				tc.test(t, newVerticalMergeSeriesIterator(
...
func (tc chunkIteratorCase) test(t *testing.T, it storage.ChunkIterator) {
...
func TestChunkIterators(t *testing.T) {
	...
	t.Run("Simple", func(t *testing.T) {
		for i, tc := range cases {
			t.Run(fmt.Sprintf("%v", i), func(t *testing.T) {
				tc.test(t, storage.NewChunkIterator(append(append(append([]chunks.Meta{}, tc.a...), tc.b...), tc.c...)))
			})
		}
	})
	...
	t.Run("Vertical", func(t *testing.T) {
		...
		for i, tc := range append(cases, overlappingCases...) {
			t.Run(fmt.Sprintf("%v", i), func(t *testing.T) {
				tc.test(t, newVerticalMergeChunkIterator(
					&mockSeries{chunkIterator: func() storage.ChunkIterator { return storage.NewChunkIterator(tc.a) }},
					&mockSeries{chunkIterator: func() storage.ChunkIterator { return storage.NewChunkIterator(tc.b) }},
					&mockSeries{chunkIterator: func() storage.ChunkIterator { return storage.NewChunkIterator(tc.c) }},
				))
			})
		}
	})
...
func BenchmarkMergedSeriesSet(b *testing.B) {
	var sel = func(sets []storage.SeriesSet) storage.SeriesSet {
		return storage.NewMergeSeriesSet(sets...)
	}
...
type mockSeries struct {
	...
	chunkIterator func() storage.ChunkIterator
}
...
func newSeries(l map[string]string, s [][]tsdbutil.Sample) storage.Series {
	...
	return &mockSeries{
		...
		chunkIterator: func() storage.ChunkIterator { return storage.NewChunkIterator(cs) },
	}
}

func (m *mockSeries) Labels() labels.Labels                { return m.labels() }
func (m *mockSeries) Iterator() chunkenc.Iterator          { return m.iterator() }
func (m *mockSeries) ChunkIterator() storage.ChunkIterator { return m.chunkIterator() }
	...
	for _, c := range cases {
		for _, overlapPercentage := range c.overlapPercentages {
			...
			b.Run(benchMsg, func(b *testing.B) {
				...
				ss, _, err := sq.Select(nil, labels.MustNewMatcher(labels.MatchRegexp, "__name__", ".*"))
				for ss.Next() {
					it := ss.At().Iterator()
					for t := mint; t <= maxt; t++ {
						it.Seek(t)
...
func BenchmarkSetMatcher(b *testing.B) {
	...
	for _, c := range cases {
		...
		b.Run(benchMsg, func(b *testing.B) {
			...
			b.ReportAllocs()
			for n := 0; n < b.N; n++ {
				_, _, err := que.Select(nil, labels.MustNewMatcher(labels.MatchRegexp, "test", c.pattern))
				testutil.Ok(b, err)
...
func benchQuery(b *testing.B, expExpansions int, q storage.Querier, selectors labels.Selector) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		ss, _, err := q.Select(nil, selectors...)
		testutil.Ok(b, err)
